...
int send_i2c(int slave_addr, char data[], int size)
{
  /* Send char array */
  Serial.println("Sending CHAR array");
  Wire.beginTransmission(slave_addr);
...
{
  delay(2000);
  Serial.println("Master | Write data to slave");

  communicationStruct my_s;
  
  my_s.boolValue = "MS";
  my_s.integer = 1234;
  
  char* my_s_bytes = reinterpret_cast<char*>(&my_s);
  Serial.println(my_s_bytes);
  Serial.println(sizeof(my_s_bytes));
  Serial.println(sizeof(my_s_bytes[0]));

  // Write a charatre to the Slave
  // send_i2c(SLAVE_ADDR, *dataArray, 5);
  send_i2c(SLAVE_ADDR, my_s_bytes, sizeof(my_s_bytes)/sizeof(my_s_bytes[0])); // pass pointer to the array and its size
  // send_i2c(SLAVE_ADDR, intArray, sizeof(intArray)/sizeof(intArray[0])); // pass pointer to the array and its size

  Serial.println("Master | Going to receive data");
